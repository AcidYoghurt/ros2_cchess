...
class ChessEngineNode(Node):
    def __init__(self):
        ...
        self.get_logger().info("ROS 2 中国象棋引擎节点已启动。等待UCI配置信息...")
        ...
        # --- ROS 通信 ---
        self.uci_subscription = self.create_subscription(String, 'uci_topic', self.uci_callback, 10)
        self.move_subscription = self.create_subscription(String, 'chess_move_topic', self.move_callback, 10)
        self.ai_move_publisher = self.create_publisher(String, '/right/ai_move_topic', 10)
    ...
    def move_callback(self, msg):
        ...
        try:
            ...
            if len(parts) < 2:
                self.get_logger().error(f"消息格式错误: {msg.data}")
                wrong_msg = String()
                wrong_msg.data = "error:wrong_move_msg"
                self.status_publisher.publish(wrong_msg)
                return

            move_part = parts[1]
            move_uci = move_part.split(': ')[1] if ': ' in move_part else move_part

            success, message, outcome = self.game.make_player_move(move_uci)
            if not success:
                self.get_logger().warn(message)
                s = String()
                s.data = f"error:illegal_move:{message}"
                self.status_publisher.publish(s)
                return
    ...
    def handle_config_message(self, config_data):
        ...
        if self.init_game():
            if self.config['player_color'] == cchess.BLACK:
                self.get_logger().info("玩家执黑，引擎先行。")
                self.execute_engine_move()
            else:
                self.get_logger().info("玩家执红，发布初始棋盘状态。")
                # 发布一次初始棋盘的FEN
                self.publish_fen()
    ...
    def execute_engine_move(self):
        ...
        s = String(); s.data = "info:机械臂1引擎思考中。。。"
        self.status_publisher.publish(s)
    ...
    def handle_game_over(self, outcome):
        """处理游戏结束，自动检测将死"""
        result_msg = f"over:{outcome.result()}"
        self.get_logger().info(result_msg)

        # 如果是将死，则额外通知前端
        if outcome.termination.name.upper() == "CHECKMATE":
            checkmate_msg = String()
            winner = "红方" if outcome.result() == "RED_WIN" else "黑方"  # 根据返回的胜利方标识调整
            checkmate_msg.data = f"将死！胜利方: {winner}"
